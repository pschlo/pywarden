...
class ApiControl:
    ...
    def create(process: Popen, host: str, port: int) -> ApiControl:
        state = ApiState(host=host, port=port)
    ...
    def shutdown(self, timeout_secs: float | None = 10) -> None:
        ...
        try:
            ...
            self.process.kill()
            self.process.wait()
            raise TimeoutError(f"API server did not exit after {timeout_secs} seconds") from error

    def wait_until_ready(self, timeout_secs: float | None = 10) -> None:
        ...
        while timeout_secs is None or time.perf_counter() - t0 < timeout_secs:
            if not self.is_alive():
                ...
                log.error(stderr)
                raise TimeoutError("API server process is dead")
