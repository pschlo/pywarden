...
class BaseBwControl:
    ...
    def __init__(
        self, cli: CliControl | CliConfig | None = None, api: ApiConfig | None = None
    ) -> None:
        ...
        if isinstance(cli, CliConfig):
            ...
            if conf.cli_path is None:
                ...
                r = shutil.which("bw")
                if r is None:
                    raise ValueError("Could not locate Bitwarden CLI executable")
                conf.cli_path = Path(r)
            cli = CliControl.create(
                cli_path=conf.cli_path, data_dir=conf.data_dir, server=conf.server
            )
    ...
    def login_unlock_interactive(self, email: str | None = None) -> Iterator[UnlockedBwControl]:
        ...
        if (
            ...
            and cast(AuthStatusResponse, status)["userEmail"] == email
            and status["serverUrl"] == self.cli.get_server()
        ):
            # already logged in
